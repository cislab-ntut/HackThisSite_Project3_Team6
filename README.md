...
### 額外補充：
1. 報告完後老師提供新的想法，去修改這個程式傳送的封包。
但是在我安裝玩Charles，之後並且嘗試抓取這個封包，卻發現Charles裡面的目錄找不到有包含Hackthissite網址的任何封包，

![](app_challenge3/額外補充-1.jpg)
2. 另外因為其他組員的解題過程中幾乎都是使用反組譯器來實作的，因此我也想說來試試看能不能用反組譯器來操作，
但是打開Ollydbg之後發現裡面並沒有看到類似剛剛的true和false的東西可以修改。

![](app_challenge3/額外補充-2.jpg)
